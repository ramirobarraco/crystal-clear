...
(define (sat-test-suite)
  ...
  (test-equal (term (comp-varsol (x △ y)))
              (term x)
              )
  (test-equal (term (comp-varsol (x △ (1 + 1))))
              (term x)
              )
  (test-equal (term (comp-varsol (x ⊔ y)))
              (term (not (x ⊔ y)))
  ...
  (test-equal (term (comp-SOL (x : (x ⊓ y) ·)))
              (term (x : (not (x ⊓ y)) ·))
              )
  (test-equal (term (comp-SOL (x : (x △ y) ·)))
              (term (x : x ·))
  ...
  (test-equal (term (denote-varsol Bool))
              (term Bool)
              )

  (test-equal (term (denote-varsol Int32))
              (term Int32)
              )

  (test-equal (term (denote-varsol (Bool Int32)))
              (term (Bool Int32))
              )

  (test-equal (term (denote-varsol (not Bool)))
              (term (Nil Int32 String))
              )

  (test-equal (term (denote-varsol ((not (Nil Int32 String)) ⊔ String)))
              (term (Bool String))
              )

  (test-equal (term (denote-varsol ((Nil Int32 String) ⊔ String)))
              (term (Nil Int32 String))
              )
  ...
  (test-equal (term (denote-varsol (String △ (not (Nil Int32 String)))))
              (term ⊥)
              )

  (test-equal (term (denote-varsol (String △ (Nil Int32 String))))
              (term String)
              )

  ; instantiation of SOL
  (test-equal (term (denote-SOL (y : Bool ·)))
              (term (y : Bool ·))
              )

  (test-equal (term (denote-SOL (y : Int32 ·)))
              (term (y : Int32 ·))
              )

  (test-equal (term (denote-SOL (y : (Bool Int32) ·)))
              (term (y : (Bool Int32) ·))
              )

  (test-equal (term (denote-SOL (y : (not Bool) ·)))
              (term (y : (Nil Int32 String) ·))
              )

  (test-equal (term (denote-SOL (z : ((not (Nil Int32 String)) ⊔ String) ·)))
              (term (z : (Bool String) ·))
              )
  ...
  (test-equal (judgment-holds (SAT ((x == y) and (isa? String x))
                                   (x : ((x △ y) ⊓ String)
                                      (y : ((y △ x) ⊓ y) ·)))) #t)
  (test-equal (judgment-holds (SAT (x < y) ·)) #t)
  (test-equal (judgment-holds (SAT ((x < y) or (isa? String x))
                                   (x : (x ⊔ String) ·))) #t)
  (test-equal (judgment-holds (SAT (x <= y) ·)) #t)
  (test-equal (judgment-holds (SAT (x > y) ·)) #t)
  (test-equal (judgment-holds (SAT (x >= y) ·)) #t)
  (test-equal (judgment-holds (SAT (x < 1) ·)) #t)
  (test-equal (judgment-holds (SAT (x < (1 + 1)) ·)) #t)
  (test-equal (judgment-holds (SAT (x < (true or false)) ·)) #t)
  (test-equal (judgment-holds (SAT (x < (not (1 + 1))) ·)) #t)
